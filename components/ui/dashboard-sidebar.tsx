        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          {activeId === "api" || activeId === "home" ? (
            ...
            <div className="p-8 rounded-2xl border border-black/[0.08] bg-white text-center font-sans space-y-3">
              ...
              <p className="text-xs text-black/60">Selecciona el módulo <strong className="text-[#111]">Fuerza Agéntica IA</strong> o <strong className="text-[#111]">Configuración</strong> en el menú lateral.</p>
            </div>
          )}
...
// ── COMPONENTE: CONFIGURACIÓN DE MODELOS DE IA & GOOGLE GEMINI ──────────────────
function SettingsAiModule() {
  ...
  useEffect(() => {
    const savedGemini = localStorage.getItem("SC_GEMINI_API_KEY") || ""
    const savedOpenAi = localStorage.getItem("SC_OPENAI_API_KEY") || ""
    if (savedGemini) {
  ...
  const handleConnectGemini = async () => {
    if (!geminiKey.trim()) {
      setStatusMessage("Ingresa una llave de API de Google Gemini.")
      return
    }

    setIsConnecting(true)
    setStatusMessage("Verificando credenciales con la API de Google Gemini...")
    ...
    try {
      ...
      if (data.success) {
        localStorage.setItem("SC_GEMINI_API_KEY", geminiKey)
        setConnectionStatus("connected")
      ...
      setStatusMessage("Error conectando con la API de verificación de llaves.")
    } finally {
  ...
  return (
    <div className="space-y-6 font-sans text-xs max-w-4xl animate-in fade-in">
      <div className="pb-4 border-b border-black/[0.08]">
        <h1 className="text-2xl font-bold text-[#111]">Configuración de Modelos de IA & Conexión Gemini</h1>
        <p className="text-xs text-black/60 mt-1">
          Conecta tu llave oficial de Google Gemini API para habilitar el razonamiento agéntico y copywriting B2B en tiempo real.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* CARD GOOGLE GEMINI */}
        <div className="p-6 rounded-2xl border border-black/10 bg-white shadow-2xs space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-purple-900 text-white flex items-center justify-center font-bold">
                <Sparkles className="w-4 h-4 text-purple-200" />
              </div>
              <div>
                <h3 className="text-sm font-bold text-[#111]">Google Gemini AI API</h3>
                <span className="text-[10px] font-mono text-black/50">Google DeepMind</span>
              </div>
            </div>
            <span className={`px-2.5 py-0.5 rounded-full font-mono text-[10px] font-bold ${
              ...
              {connectionStatus === 'connected' ? '✓ CONECTADO' : 'PENDIENTE'}
            </span>
          </div>

          <div className="space-y-3">
            <div>
              <label className="text-[11px] font-semibold text-[#111] block mb-1">Modelo Seleccionado:</label>
              <select
                value={selectedModel}
                onChange={(e) => setSelectedModel(e.target.value)}
                className="w-full p-2.5 rounded-xl bg-[#F5F4F0] border border-black/10 text-xs font-semibold text-[#111] outline-none"
              >
                <option value="gemini-2.5-flash">Gemini 2.5 Flash (Recomendado — Alta Velocidad & RAG)</option>
                <option value="gemini-1.5-pro">Gemini 1.5 Pro (Razonamiento Complejo B2B)</option>
                <option value="gemini-2.0-flash">Gemini 2.0 Flash (Experimental)</option>
              </select>
            </div>

            <div>
              <label className="text-[11px] font-semibold text-[#111] block mb-1">Llave de API (GEMINI_API_KEY):</label>
              <div className="relative">
                <Key className="w-4 h-4 text-black/40 absolute left-3 top-3" />
                <input
                  type="password"
                  placeholder="AIzaSy..."
                  value={geminiKey}
                  onChange={(e) => setGeminiKey(e.target.value)}
                  className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white border border-black/10 text-xs outline-none focus:border-purple-600 font-mono"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-purple-900 text-white font-semibold text-xs hover:bg-purple-950 flex items-center justify-center gap-2 cursor-pointer transition-all shadow-2xs disabled:opacity-50"
            >
              <Zap className="w-4 h-4 text-purple-300" />
              <span>{isConnecting ? "Verificando Credencial..." : "Probar & Guardar Conexión Gemini"}</span>
            </button>

            {statusMessage && (
              <p className={`text-[11px] font-mono p-2.5 rounded-xl border ${
                statusMessage.includes('✓') ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-rose-50 text-rose-800 border-rose-200'
              }`}>
                {statusMessage}
              </p>
            )}
        ...
        {/* CARD POLÍTICAS Y AUTONOMÍA */}
        <div className="p-6 rounded-2xl border border-black/10 bg-white shadow-2xs space-y-4">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-indigo-900 text-white flex items-center justify-center font-bold">
              <ShieldCheck className="w-4 h-4 text-indigo-200" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-[#111]">Gobernanza & Políticas de Autonomía</h3>
              <span className="text-[10px] font-mono text-black/50">Automatización Supabase RLS</span>
            </div>
          </div>

          <div className="space-y-3 font-mono text-[11px]">
            <div className="p-3 rounded-xl bg-[#F5F4F0] border border-black/[0.06] flex items-center justify-between">
              <span className="text-black/60">Límite Diario de Despacho:</span>
              <span className="font-bold text-[#111]">2,000 Correos / 24h</span>
            </div>
            <div className="p-3 rounded-xl bg-[#F5F4F0] border border-black/[0.06] flex items-center justify-between">
              <span className="text-black/60">Aprobación Humana (Human-in-the-Loop):</span>
              <span className="font-bold text-purple-800">OBLIGATORIA (Level 1)</span>
            </div>
            <div className="p-3 rounded-xl bg-[#F5F4F0] border border-black/[0.06] flex items-center justify-between">
              <span className="text-black/60">Tasa Máxima de Queja de Spam:</span>
              <span className="font-bold text-rose-700">0.10% Max (Gmail Standard)</span>
            </div>
            <div className="p-3 rounded-xl bg-[#F5F4F0] border border-black/[0.06] flex items-center justify-between">
              <span className="text-black/60">Supresión Habeas Data Ley 1581:</span>
              <span className="font-bold text-emerald-800">Global e Inmutable</span>
            </div>
          </div>
        </div>
      </div>
...
// ── COMPONENTE: FUERZA AGÉNTICA IA — ORQUESTRACIÓN MULTI-AGENTE ─────────────────
function FuerzaAgenticaModule() {
  const [selectedAgent, setSelectedAgent] = useState("email_specialist")
  const [chatMessages, setChatMessages] = useState<Array<{ role: 'user' | 'agent'; text: string; time: string }>>([
    {
      role: 'agent',
      text: '¡Hola! Soy el Agente Especialista en Email Marketing y Orquestación Agéntica. ¿Qué estrategia o categoría comercial deseas analizar hoy?',
  ...
  const agents = [
    { id: "email_specialist", name: "Agente Especialista en Email Marketing", role: "Estrategia RAG & 5 Ángulos Comerciales", status: "Activo" },
    { id: "copywriter", name: "Agente de Copywriting B2B & Facts", role: "Redacción Sanitizada basada en CONTEXT.md", status: "Activo" },
    { id: "validator", name: "Validador Determinista & Habeas Data", role: "Verificación de Enlaces, Baja & Hash SHA-256", status: "Activo" },
    { id: "dispatcher", name: "Worker de Despacho Asíncrono", role: "Colas de Supabase & Goteo Antispam", status: "Activo" },
    { id: "learner", name: "Agente de Autoaprendizaje & RAG", role: "Evaluación de Candidatos y Promoción RPC", status: "Activo" },
  ]
  ...
  const handleSendMessage = async () => {
    ...
    setChatMessages((prev) => [...prev, { role: 'user', text: promptText, time }])
    setUserInput("")
    ...
    try {
      ...
      if (data.success) {
        setChatMessages((prev) => [
          ...
          {
            role: 'agent',
            text: `He analizado tu instrucción para "${promptText}". Consulté la memoria RAG de Supabase (Scores: Objetivo 92%, Audiencia 88%) y estructuré 5 variaciones congeladas con Release Hash SHA-256: ${data.release_hash}. Puedes revisarlas y aprobarlas en la consola.`,
            time: new Date().toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }),
          },
        ])
      } else {
        setChatMessages((prev) => [
          ...
          {
            role: 'agent',
            text: `Inconveniente detectado por el Validador: ${data.error}`,
            time: new Date().toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }),
          },
        ])
      }
      ...
      setChatMessages((prev) => [
        ...
        {
          role: 'agent',
          text: 'Fallo de conexión con el Orquestador Agéntico.',
          time: new Date().toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }),
        },
      ])
    } finally {
  ...
  return (
    <div className="space-y-6 font-sans text-xs animate-in fade-in">
      <div className="pb-4 border-b border-black/[0.08] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#111]">Fuerza Agéntica IA — Centro de Orquestación Multi-Agente</h1>
          <p className="text-xs text-black/60 mt-0.5">
            Interactúa en tiempo real con tus agentes especializados de inteligencia artificial.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono bg-purple-100 text-purple-900 px-3 py-1 rounded-full font-bold">
            Motor: Google Gemini 2.5 Flash / RAG Supabase
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* PANEL IZQUIERDO: SELECCIÓN DE AGENTE ESPECIALIZADO */}
        <div className="space-y-3">
          <span className="text-[10px] font-mono font-bold text-black/50 uppercase tracking-widest block">
            UNIDADES AGÉNTICAS DISPONIBLES ({agents.length})
          </span>
              ...
              return (
                <div
                  ...
                  className={`p-3.5 rounded-2xl border transition-all cursor-pointer ${
                    isSelected ? "bg-purple-950 text-white border-purple-900 shadow-md" : "bg-white border-black/10 hover:border-black/20 text-[#111]"
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-xs">{agent.name}</span>
                    <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded-full ${
                      isSelected ? "bg-purple-800 text-white" : "bg-emerald-100 text-emerald-800"
                    }`}>
                      {agent.status}
                    </span>
                  </div>
                  <p className={`text-[11px] ${isSelected ? "text-purple-200" : "text-black/60"}`}>{agent.role}</p>
                </div>
              )
            })}
          </div>
        ...
        {/* PANEL DERECHO: INTERFACING Y DIÁLOGO CON EL AGENTE SELECCIONADO */}
        <div className="lg:col-span-2 bg-white rounded-2xl border border-black/10 p-5 shadow-2xs flex flex-col justify-between min-h-[460px]">
          <div className="border-b border-black/[0.08] pb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] font-mono text-black/40">Modo Conversacional Agéntico Activo</span>
              </div>
            </div>
          ...
          <div className="my-4 space-y-3 max-h-80 overflow-y-auto pr-2">
            {chatMessages.map((msg, idx) => (
              ...
              >
                <div className={`max-w-[85%] p-3.5 rounded-2xl text-xs ${
                  msg.role === 'user'
                    ? 'bg-purple-900 text-white font-medium rounded-br-none'
                    : 'bg-[#F5F4F0] border border-black/[0.06] text-[#111] rounded-bl-none'
                }`}>
                  <p className="whitespace-pre-wrap">{msg.text}</p>
                </div>
                <span className="text-[9px] font-mono text-black/40 mt-1 px-1">{msg.time}</span>
              </div>
            ))}
          </div>
          ...
          <div className="pt-3 border-t border-black/[0.08] flex items-center gap-2">
            <input
              ...
              placeholder="Escribe tu requerimiento para el agente (ej: Analiza la campaña para admisiones y propone 5 textos)..."
              className="flex-1 px-4 py-2.5 rounded-xl bg-[#F5F4F0] border border-black/10 text-xs text-[#111] outline-none focus:border-purple-600 font-sans"
            />
            <button
              onClick={handleSendMessage}
              disabled={isProcessing}
              className="px-4 py-2.5 rounded-xl bg-purple-900 text-white font-semibold text-xs hover:bg-purple-950 flex items-center gap-1.5 cursor-pointer shadow-2xs disabled:opacity-50"
            >
