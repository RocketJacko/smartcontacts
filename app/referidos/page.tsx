  ...
  const validateEmailDomain = async (inputEmail: string) => {
    ...
    try {
      ...
      if (data && data.valid === false) {
        setEmailError(data.message || "Email no aceptado (dominio bloqueado)")
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full px-3 py-2 rounded-xl text-xs outline-none transition-all ${
                        emailError
                          ? "bg-red-50/20 border border-red-500 focus:border-red-600"
                          : "bg-[#F5F4F0] border border-black/10 focus:border-black/30"
                      }`}
                    />
                    {isValidatingEmail && (
